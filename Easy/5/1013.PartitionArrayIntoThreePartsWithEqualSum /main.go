...
func canThreePartsEqualSum(arr []int) bool {
	sumSlice := float64(getSumSlice(arr))
	sumSlice3 := sumSlice / 3
	sum_ := 0
	i := 0
	for i = 0; i < len(arr); i++ {
		sum_ += arr[i]
		if math.Abs(float64(sum_)-sumSlice3) < 0.1 {
			sum_ = 0
			break
		}
	}
